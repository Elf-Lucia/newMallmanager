  ...
  methods: {
    ...
    async handleDelete(catId) {
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(async () => {
          // 发送删除的网络请求
          const { resData } = await this.$http.delete(`categories/${catId}`);
    ...
    async handleAdd() {
      ...
      // 发送请求 返回201 则添加
      const resData = await this.$http.post(`categories`, this.form);
    ...
    async handleShowAddDialog() {
      ...
      // 加载层级数据
      const resData = await this.$http.get(`categories?type=2`);
      console.log(resData);
